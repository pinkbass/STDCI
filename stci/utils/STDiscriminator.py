...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        self.main_stream.add_module(
            'initial_conv',
            nn.Conv3d(in_channels, base_dim, kernel_size=4, stride=2, padding=1) # 4x4x4 kernel, stride 2, reduces T/H/W by half
        )
        ...
        for i in range(1, num_layers):
            next_dim = min(current_dim * 2, 512) # 特征维度翻倍，上限512或1024
            self.main_stream.add_module(
                f'down_block_{i}',
                nn.Sequential(
        ...
        self.output_conv = nn.Conv3d(current_dim, 1, kernel_size=4, stride=1, padding=1) # 输出1通道的logit图
    ...
    def forward(self, x, template_cond=None):
        ...
        if self.cond_net is not None and template_cond is not None:
            # 确保模板条件与当前特征图的空间/时间尺寸匹配
            # 假设 template_cond 已经过预处理或编码到合适的尺寸
            # 或者你需要在cond_net中处理其尺寸匹配问题
            cond_features = self.cond_net(template_cond)
            # 可能需要resize cond_features 以匹配 main_features 的尺寸
            if cond_features.shape[2:] != main_features.shape[2:]:
                 # 使用3D插值进行尺寸匹配
                cond_features = torch.nn.functional.interpolate(
                    cond_features,
